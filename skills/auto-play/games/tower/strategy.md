...
- 2026-09-15 巨人战士实测推进到 16 层：`巨人协议 + 二星撞击` 的伤害方向正确，`迅捷攻击` 能同时补伤害与抽牌，但 `未来汽水` 只回蓝不抽牌；牌库变厚后核心集中到第 5 回合才出现，普通战也要 4-5 回合。休息回复会被无盾的一场战斗迅速吃完，因此必须保留最后一张 `举盾` 与 `防具加固` 的依赖关系，并继续优先真实回血、抽牌、赚钱，再补爆发；单纯提高生命上限不能替代当前生命恢复。
