...
- 2026-09-15 巨人战士首局实测止步 20 层：`巨人协议 + 二星撞击` 的伤害方向正确，`迅捷攻击` 重复后合成三星，实测为 6 点物理伤害并抽 1 张；`未来汽水` 只回蓝不抽牌。牌库变厚后核心集中到第 5 回合才出现，普通战也要 4-5 回合。20 层 Boss 战前只剩 3/432 生命，手中没有即时护盾或回血，证明休息回复会被无盾的连续战斗迅速吃完。必须保留最后一张 `举盾` 与 `防具加固` 的依赖关系；`守势` 的 2 点护盾加抽 1 能同时补防线与循环，应高于普通攻击牌。`启动防守` 在无盾时提供 6 点护盾且使用后移除，也应作为一次性防守核心而非换成 10 金币。继续优先真实回血、抽牌、赚钱，再补爆发；单纯提高生命上限不能替代当前生命恢复。
- 同次实测中，15 层火星蚊曾击杀角色；广告复活后重开同一战斗，`巨人协议` 把生命上限抬到约 322、当前生命约 122，才获得继续恢复的空间。广告复活可作允许的兜底，但路线仍需在死亡前补足即时护盾或回血。
- `扶虎斧` 的实测效果是敌人使用非物理、非法术牌时使我方物攻 +8，巨人战士可以在水晶充足时购买。`假笑面具II` 会在每场战斗开始时把一张 `跌倒` 加入抽牌堆，明显污染循环；后续遗忘房或魔法商店应优先移除 `跌倒`。
- 训练师的“升级时获得 100 金币”任务不会直接塞牌，是厚牌未成型阶段的安全经济来源。低血道具口袋若没有真实回血，可用 `唤回药水` 回收高伤害移除牌争取当回合斩杀，但不能把它计作续航。
- 从维护或中断恢复时，本地楼层可能落后于游戏 HUD；应让屏幕上的实际楼层覆盖旧状态。商店界面的 `已刷新N次` 同样必须覆盖本地刷新计数，达到两次立即离开；战前面板中 `战斗` 必须高于敌人名称，避免把怪物误判成地图房间。
...
- **巨人药水**: shop card, cost 0, consumable; description said it increases max
  HP by `600` for the current battle only; live testing showed the bonus disappears
  after combat and does not raise current HP by itself. Do not spend scarce gold on
  it ahead of real healing or cycle cards. Save it for a late boss and pair it with
  same-battle percentage healing such as **深澜龙血** so the larger max HP converts
  into actual survivability.
