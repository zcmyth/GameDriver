...
### 2026-09-15 巨人战士深渊实测
- `巨人之拳` 开局后，水晶商店买到 `魔法熊手`；使用 `迅捷` 这类瞬发牌时会免费从抽牌堆打出一张物攻或法术牌。实战顺序应为先装备 `攻击宝石`，再打 `迅捷` 触发熊手，最后手动打剩余攻击牌。
- 水晶商店的免费次数文字位于刷新按钮上方，点击文字坐标不会刷新；稳定点击区约为归一化坐标 `(0.14, 0.303)`。每店最多刷新两次，优先赚钱、永久生命/回血、抽牌回蓝和当前构筑联动。
- `诅咒饭团` 拾取时减少 25 点生命上限，此后每场胜利永久增加 5 点生命上限；第 10 层取得时约五场回本，适合长线冲 124 层，但取得后应就近休息抵消前期亏血。
- 训练师的 `超时空之手` 任务只需击败 3 只普通怪，奖励为洗牌时生成一张随机临时宝石。它不会永久塞牌，并能触发宝石与熊手联动，优先于告别；直接获得 2 张技能牌或 4 张一级牌的任务仍应跳过。
- 实机亮着可用的 `迅捷` OCR 可点击度为 1.684，而法力不足的暗色攻击牌约为 1.13；亮牌阈值校准为 1.55，避免把可用循环牌误判为禁用。
- 第 20 层奶龙鹿战中，角色降到 `54/225` 时使用 `深澜龙血`，在 `实验眼镜` 复制道具效果后恢复至 `98/225`，实际回复 44 点，吻合两次 10% 最大生命回复。随后 `迅捷药水` 抽空剩余牌库，`全力一击 -> 制造核心` 将敌人从 88 压到 11，最后由 `神圣斩击` 收头并触发永久生命成长。自动战斗因此增加低于约 30% 生命且道具数不为零时的保命规则：回血消耗品优先，抽牌消耗品作为爆发兜底。
